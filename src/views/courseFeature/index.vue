<template>
  <div class="app-container">
    ...
    <el-table v-loading="tableLoading" :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="title" label="课程标题" min-width="160" :align="'center'" fixed />
      ...
      <el-table-column prop="courseFeatureColumnName" label="专题分类" min-width="160" :align="'center'" />
    ...
    <el-dialog :title="editForm.id ? '编辑课程' : '新增课程' " :visible.sync="dialogVisible" width="1000px" @close="dialogClose">
      <el-form ref="form" :model="editForm" :rules="rules">
        <el-form-item label="课程标题：" prop="title" label-width="140px">
          <el-input v-model="editForm.title" style="width: 400px" />
        </el-form-item>
        <el-form-item label="一级分类：" prop="featureCategoryId1" label-width="140px">
          <el-select
        ...
        <el-form-item label="二级分类：" prop="featureCategoryId2" label-width="140px">
          <el-select
            v-model="editForm.featureCategoryId2"
            style="width: 370px"
            placeholder="请选择"
            :disabled="!editForm.featureCategoryId1"
        ...
        <el-form-item label="专题分类：" prop="courseFeatureColumnId" label-width="140px">
          <el-select
            v-model="editForm.courseFeatureColumnId"
            style="width: 370px"
            placeholder="请选择"
            :disabled="!editForm.featureCategoryId2"
          >
            <el-option
        ...
        <el-form-item label="课程封面：" prop="imgUrl" label-width="140px">
          <el-upload
            ref="upload"
            :limit="1"
            :auto-upload="true"
        ...
        <el-form-item label="课程内容：" prop="context" label-width="140px">
          <editor-bar :value="editForm.context" @change="change" />
        </el-form-item>
        <el-form-item label="课程简介：" prop="remark" label-width="140px">
...
import { getList, getFeatureCategoryListById, getColumnListById, getDetail, edit, add, del } from '@/api/courseFeature'
import pagination from '@/components/Pagination'
import EditorBar from './text'
export default {
  name: 'CourseFeatureIndex',
  components: { pagination, EditorBar },
  data() {
    return {
      headers: { Authorization: getToken() },
      permission: {
        add: ['admin', 'courseFeature:add'],
        edit: ['admin', 'courseFeature:edit'],
        del: ['admin', 'courseFeature:del']
      },
      currentPage: 1,
      pageSize: 20,
      total: 0,
      ...
      editForm: {
        ...
        courseFeatureColumnId: null,
        courseFeatureColumnName: null
      },
      dialogVisible: false,
      rules: {
        title: [
  ...
  methods: {
    ...
    getSourceCategory(val) {
      this.editForm.courseFeatureColumnId = null
      this.editForm.courseFeatureColumnName = null
      this.columnList = []
    ...
    getColumnList(val) {
      ...
      getColumnListById(val).then(res => {
        if (res.length < 1) {
          this.$message.error('该分类下暂时还没有专题，请重新选择或添加专题！')
        } else {
          this.columnList = res
        }
        this.editForm.featureCategoryName2 = this.twoCategoryList.filter(item => item.id === val)[0]?.name
      })
    },
    selectColumn(val) {
      this.editForm.courseFeatureColumnName = this.columnList.filter(item => item.id === val)[0]?.title
    },
    // 获取专题分类
    getTableData() {
      const req = {
    ...
    getEdit(id) {
      ...
      getDetail({ id: id }).then(res => {
        if (res.featureCategoryId1 > 0) {
          getFeatureCategoryListById(res.featureCategoryId1).then(res => {
            this.twoCategoryList = res
          })
        }
        if (res.featureCategoryId2 > 0) {
          getColumnListById(res.featureCategoryId2).then(res => {
            this.columnList = res
          })
        }
        this.editForm = res
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          const { title, id,
            ...
            featureCategoryId1,
            featureCategoryId2,
            courseFeatureColumnId } = this.editForm
          ...
          const courseFeatureColumnName = this.columnList.filter(item => item.id === this.editForm.courseFeatureColumnId)[0]?.title
          ...
          req({ title, id,
            ...
            featureCategoryId1,
            featureCategoryId2,
            courseFeatureColumnId, featureCategoryName1, featureCategoryName2, courseFeatureColumnName }).then(res => {
