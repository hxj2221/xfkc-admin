<template>
  <div class="app-container">
    ...
    <el-table v-loading="tableLoading" :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="name" label="标题" min-width="160" :align="'center'" fixed />
    ...
    <el-dialog :title="editForm.id ? '编辑课程资源分类' : '新增课程资源分类' " :visible.sync="dialogVisible" width="600px" @close="dialogClose">
      <el-form ref="form" :model="editForm" :rules="rules">
        <el-form-item label="分类标题：" prop="name" label-width="160px">
          <el-input v-model="editForm.name" style="width: 370px" />
        </el-form-item>
...
import { getSourceCategorList, getSourceCategoryListById, getSourceCategoryDetail, editSourceCategory, addSourceCategory, delSourceCategory } from '@/api/sourceCategory'
import pagination from '@/components/Pagination'
export default {
  name: 'SourceCategoryClass',
  components: { pagination },
  data() {
    return {
      headers: { Authorization: getToken() },
      permission: {
        add: ['admin', 'sourceCategory:add'],
        edit: ['admin', 'sourceCategory:edit'],
        del: ['admin', 'sourceCategory:del']
      },
      currentPage: 1,
      pageSize: 20,
      total: 0,
  ...
  methods: {
    // 获取分类
    getSourceCategory() {
      getSourceCategoryListById(0).then(res => {
        if (res) {
    ...
    getTableData() {
      ...
      getSourceCategorList(req).then(res => {
    ...
    getEdit(id) {
      ...
      getSourceCategoryDetail({ id: id }).then(res => {
        this.editForm = res
        this.dialogVisible = true
      })
    },
    ...
    save() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const req = this.editForm.id ? editSourceCategory : addSourceCategory
